...
public class DemoDataGenerator {
    ...
    @EventListener
    public void generateDemoData(ApplicationStartedEvent startupEvent) {
        ...
        for (int i = 0; i < INGREDIENT_LIST.size(); i++) {
            ...
            for (String productVariation : PRODUCT_VARIATION_LIST) {
                ingredientMap.put(
                        new Product(Long.toString(productId++), ingredient + " " + productVariation),
                        Set.of(ingredient)
                );
            }
            ingredientMap.put(new Product(
                    Long.toString(productId++),
                    ingredient + " and " + ingredientA + " " + PRODUCT_VARIATION_LIST.get(1)
            ), Set.of(ingredient, ingredientA));
            ingredientMap.put(new Product(
                    Long.toString(productId++),
                    ingredient + " and " + ingredientB + " " + PRODUCT_VARIATION_LIST.get(2)
            ), Set.of(ingredient, ingredientB));
            ingredientMap.put(new Product(
                    Long.toString(productId++),
                    ingredient + ", " + ingredientA + " and " + ingredientC + " " + PRODUCT_VARIATION_LIST.get(1)
            ...
            for (Product previousProduct : products) {
                ...
                Duration cleaningDuration = Duration.ofMinutes(product == previousProduct ? 0
                                                                       : noCleaning ? noCleaningMinutes
                                                                               : cleaningMinutesMinimum + random.nextInt(
                                                                                       cleaningMinutesMaximum - cleaningMinutesMinimum));
        ...
        log.info("scheduling demo data is operational");
    }
